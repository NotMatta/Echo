...
export const RequestComponent = (props: {friendRequest: FriendshipInitiator, declineAction: (id: string) => void, acceptAction: (id: string) => void}) => {
  return <div className="p-4 border-b border-background flex justify-between items-center">
    ...
    <div className="flex gap-2">
      <button className="bg-green-600 px-4 py-2 rounded-lg font-bold hover:bg-green-700" onClick={() => props.acceptAction(props.friendRequest.id)}>Accept</button>
      <button className="bg-red-600 px-4 py-2 rounded-lg font-bold hover:bg-red-700" onClick={() => props.declineAction(props.friendRequest.id)}>Decline</button>
    </div>
  </div>;
}
