...
const AllPage = () => {
  ...
  return(
    <div className="w-full h-0 grow max-h-full flex flex-col gap-2 overflow-y-auto">
      ...
      <p className="text-sm text-white/40 font-semibold">All - {friends.filter((friend: Friends) => friend.online).length}</p>
      {friends.filter((friend: Friends) => friend.name.includes(filter)).map((friend: Friends) =>
