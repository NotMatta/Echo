...
const UserComponent = (props: {friendRequest: FriendshipProps}) => {
  return <div className="p-4 border-b border-background flex justify-between items-center">
    ...
    <div className="flex gap-2">
      <button className="bg-green-600 px-4 py-2 rounded-lg font-bold hover:bg-green-700">Accept</button>
      <button className="bg-red-600 px-4 py-2 rounded-lg font-bold hover:bg-red-700">Decline</button>
    </div>
  </div>;
}
...
const RequestsPage = async () => {
  ...
  return <div className="p-4">
    {requestsData.ok && requestsData.data.length > 0 ? requestsData.data.map((friendRequest: FriendshipProps) => (
      <UserComponent key={friendRequest.id} friendRequest={friendRequest} />
